...
APP_NAME="weazlinspekt"
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
INSTALL_ROOT="${WEAZLINSPEKT_HOME:-"$HOME/.weazlinspekt"}"
...
(
  cd "$REPO_ROOT"
  GOCACHE="$GO_CACHE" GOMODCACHE="$GO_MOD_CACHE" go build -buildvcs=false -o "$BIN_PATH" ./cmd/weazlinspekt
)

chmod 0755 "$BIN_PATH"

path_line='export PATH="$HOME/.weazlinspekt/bin:$PATH"'
marker_begin="# >>> weazlinspekt path >>>"
marker_end="# <<< weazlinspekt path <<<"
...
(
  cd "$REPO_ROOT"
  GOCACHE="$GO_CACHE" GOMODCACHE="$GO_MOD_CACHE" go run -buildvcs=false ./cmd/weazlinspekt-setup
)

if [[ "${WEAZLINSPEKT_SKIP_LAUNCH:-}" == "1" ]]; then
  echo "Skipping first launch because WEAZLINSPEKT_SKIP_LAUNCH=1"
else
  echo ""
